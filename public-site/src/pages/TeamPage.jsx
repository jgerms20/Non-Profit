...
const advisorAreas = [
  ...
  },
  {
    ...
    description:
      'Experts in rural economic development, agriculture, and community revitalization.',
  },
  {
    icon: GraduationCap,
    area: 'Education',
      ...
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-md p-8 sm:p-10 flex flex-col sm:flex-row gap-8 items-start">
              {/* Avatar */}
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-slate-700 leading-relaxed text-base mb-6">
                  Joshua founded SCAiL in 2026 after seeing firsthand how rural South Carolina
                  communities were being left behind in the AI revolution. With a background in
                  technology and a deep connection to South Carolina, he's building SCAiL to
                  ensure AI doesn't just benefit coastal tech hubs — it reaches the communities
                  that need it most.
                </p>
                ...
                <div className="flex items-center gap-3 justify-center sm:justify-start">
                  <a
                    ...
                    aria-label="LinkedIn"
                    className="h-9 w-9 rounded-lg border border-slate-200 flex items-center justify-center text-slate-500 hover:text-brand-navy hover:border-brand-navy transition-colors duration-150"
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {boardPositions.map((position) => {
              ...
              return (
                <div
                  key={position.title}
                  className="bg-white rounded-2xl border-2 border-dashed border-slate-300 p-6 flex flex-col gap-4 hover:border-brand-teal/50 transition-colors duration-200"
                >
                  ...
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-brand-navy mt-1 mb-2">
                      {position.title}
                    </h3>
                  ...
                  <ul className="space-y-2 mt-auto">
                    {position.responsibilities.map((resp) => (
                      <li key={resp} className="flex items-start gap-2 text-xs text-slate-500">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-10 mb-10">
            {advisorAreas.map(({ icon: Icon, area, description }) => (
              <div
                key={area}
                className="bg-slate-50 rounded-2xl border border-slate-200 p-6 flex flex-col gap-3 hover:border-brand-teal/40 hover:shadow-sm transition-all duration-200"
              >
                <div className="h-11 w-11 rounded-xl bg-brand-navy flex items-center justify-center">
                  <Icon className="h-5 w-5 text-brand-teal" />
                </div>
                <h3 className="font-bold text-brand-navy text-base">{area}</h3>
          ...
          <div className="text-center">
            ...
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 text-brand-teal font-semibold text-sm hover:text-brand-navy transition-colors"
            >
              Let us know
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {openRoles.map(({ icon: Icon, title, description }) => (
              <div
                key={title}
                className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col gap-4 hover:border-brand-teal/40 hover:shadow-md transition-all duration-200"
              >
                <div className="h-12 w-12 rounded-xl bg-brand-navy flex items-center justify-center">
                ...
                <Link
                  to="/contact"
                  className="mt-auto inline-flex items-center gap-1.5 text-sm font-semibold text-brand-teal hover:text-brand-navy transition-colors"
                >
                  Get involved
                  <ArrowRight className="h-4 w-4" />
                </Link>
